...
socket.on('position', (data) => {
  ...
  playerPosition = (data / 1023) * window.innerWidth;
...
function moveBall() {
  const ballPosition = parseInt(ball.style.left) || 0;
  
  const newBallPosition = ballPosition + ballSpeed;
...
async function sendScore(score) {
    ...
    try {
          ...
          if (response.ok) {
            ...
            // Send "S" character to Arduino to turn on and off lights
            const message = score > 0 ? 'S' :
            port.write(message);
...
async function gameResult() {
        ...
        try {
          const response = ('/gameover', {
            
            method: 'POST',
            headers: {
              'Content-Type': 'application/json'
            },
            body: JSON.stringify({ gameOver: true })
          });
      
          if (response.ok) {
            console.log('Game over message sent successfully');
            // Send "L" character to Arduino to turn on the lights
            port.write('L');
          } else {
            console.error('Error sending game over message');
          }
        } catch (err) {
          console.error(err);
        }
